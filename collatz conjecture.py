...
sequence=[]
#i made the list start out as empty and new elements are added every time the while loop_stopper=1 loop runs
y=1
index=0
print("Hi, welcome to Andres' Collatz conjecture program")
learn=input("Are you familiar with collatz conjecture?(yes/no)")
if learn=="no":
    print("Collatz conjecture is basically you pick any positive integer, if its even you divide by 2, if its odd you multiply by 3 and add 1. For all positive integers, this will result in a sequence that will eventually hit 4 and be endlessly caught in the loop 4,2, and 1.")
while y>0:
    x=1
    while x!=0:
        number1=input("Try it out by entering a number")
        try:
            int(number1)
            x=0
        except:
            print("that was not a valid number")
#I added this in last minute when we taked about the try function in class.
    num1=int(number1)
...
#I want every value num1 takes on to be added to sequence
            if (num1>num2):
                num2=num1
#i want to stre the maximum of the collatz sequence to be stored in the variable num2
            else:num2=num2
        elif num1==1:
...
#149 is the maximum characters allowed in a line in IDLE so I wanted to scale the graph down by a scale of 1/10 if the maximum number exceeded the amount of characters allowed per line.
            ...
            elif not num2<300 and (total//compiler)<300 :
                print("graph of the sequence looks approximately like")
...
# If its  really big number where alot of the elements in sequence are greater than 149, the graph will still not show the fluctiation of numbers unless I scale it down even more.
            elif (num2>1000) and (total//compiler)<1000:
                print("graph of the sequence looks approximately like")
                function(sequence,50)
            elif num2>=1000:
